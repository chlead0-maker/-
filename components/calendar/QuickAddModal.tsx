...
export default function QuickAddModal({ open, onClose, date, defaultType, isAdmin, employees, currentUserId, onSuccess }: Props) {
  ...
  function reset() {
    setTitle(''); setPriority('medium'); setTaskType('daily')
    setStartTime(''); setEndTime(''); setLocation(''); setNotes('')
    setAssigneeId(currentUserId); setError(null); setType(defaultType)
  }
        ...
        <div className="flex gap-2 mb-2">
          <button type="button" onClick={() => setType('task')}
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
              type === 'task' ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}>
            🟣 할일
          </button>
          <button type="button" onClick={() => setType('event')}
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
              type === 'event' ? 'bg-sky-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}>
            🔵 일정
          </button>
        </div>
